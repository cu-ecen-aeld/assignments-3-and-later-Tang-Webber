...
void* threadfunc(void* thread_param)
{
    ...
    //wait for obtain and check
    int obtain_status = usleep(thread_func_args->wait_to_obtain_ms * 1000);
    if (obtain_status != 0) {
        ERROR_LOG("obtain_status error code: %d", obtain_status);
        thread_func_args->thread_complete_success = false;
        return thread_param;
    } 
    //obtain mutex and check
    ...
    //wait for release and check
    int release_status = usleep(thread_func_args->wait_to_obtain_ms * 1000);
    if (obtain_status != 0) {
        ERROR_LOG("release_status error code: %d", release_status);
        thread_func_args->thread_complete_success = false;
        return thread_param;
    }    
    //obtain mutex and check
